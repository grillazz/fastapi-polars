...
async def froze_data_in_frame(
    data: list[PolarsIcedSchema],
        ...
        filename_generator: FilenameGeneratorService = Depends(get_filename_generator_service)
):
    """
    ...
    if (
        ...
        _file = await filename_generator.generate_filename()  # Generate a filename for the dump
...
async def materialize_iced_data(request: Request, s3: S3Service = Depends(), filename_generator: FilenameGeneratorService = Depends(get_filename_generator_service)):
    _file = await filename_generator.generate_filename()  # Generate a filename for the dump
